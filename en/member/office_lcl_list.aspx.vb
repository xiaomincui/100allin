...
Partial Class en_member_office_fcl_list
    ...
    Sub LoadRecord(ByVal page As String)
        ...
        If destport = "" Then
            '无目的港搜索的select语句
            sql = "select top 10 "
            sql = sql & "id,startport,title,entitle,hangxianmiaoshu,"
            sql = sql & "postdate,"
            sql = sql & "enddate "
            sql = sql & "FROM (SELECT "
            sql = sql & "id,startport,title,entitle,hangxianmiaoshu,"
        ...
        Else
            ...
            sql = sql & "enddate "
            sql = sql & "FROM (SELECT "
            sql = sql & "DISTINCT tb_yunjia.id as id,tb_yunjia.startport as startport,title,entitle,hangxianmiaoshu,"
            sql = sql & "postdate,"
            sql = sql & "CONVERT(varchar(10), enddate, 120) as enddate "
            sql = sql & "FROM "
            sql = sql & "tb_yunjia,tb_yunjiainfo "
            sql = sql & "WHERE "
            sql = sql & "ifEn=1 AND tb_yunjia.id = tb_yunjiainfo.yunjiaid "
            sql = sql & "AND (userid = " & id & " or userid2 = " & id & ") "
            sql = sql & "AND yunjialeixing = 9 "
