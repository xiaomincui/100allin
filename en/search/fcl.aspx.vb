...
Partial Class en_search_fcl
    ...
    Sub LoadRecord(ByVal page As String)
        ...
        If dp = "" And cr = "" Then
            '没有船公司及目的港时
            sql = "select top 15 "
            sql = sql & "shipcompany,sid,id,startport,entitle,hangxianmiaoshu,hits,destport,price20gp,price40gp,price40hq,price45hq,assurance,"
            sql = sql & "userid,"
            ...
            sql = sql & "(SELECT TOP " & item.ToString & " tb_yunjia.id AS id,tb_yunjia.postdate,tb_user.star_allin,(CASE WHEN TB_YunJia.enddate > GETDATE() THEN 1 ELSE 0 END) AS ifend "
            sql = sql & "FROM "
            sql = sql & "tb_yunjia,tb_user "
            sql = sql & "WHERE "
            sql = sql & "tb_yunjia.ifEn=1 AND tb_user.userid = tb_yunjia.userid "
        ...
        Else
            If dp = "" Then
                sql = "select top 15 "
                sql = sql & "shipcompany,sid,id,startport,entitle,hangxianmiaoshu,destport,price20gp,price40gp,price40hq,price45hq,price20gpvalue,price40gpvalue,price40hqvalue,price45hqvalue,assurance,"
                sql = sql & "userid,"
                sql = sql & "EN_CompanyName,"
                sql = sql & "realname,"
                sql = sql & "star_allin,"
                ...
                sql = sql & "WHERE "
                sql = sql & "V_CARRIER_PRICE.ifEnx=1 AND tb_user.userid = V_CARRIER_PRICE.useridx "
            ...
            Else
                ...
                sql = sql & "WHERE id NOT IN "
                sql = sql & "(SELECT id FROM"
                sql = sql & "(SELECT TOP " & item.ToString & " tb_yunjiainfo.id AS id,tb_yunjiainfo.postdatex as postdate,(CASE WHEN tb_yunjiainfo.enddatex > GETDATE() THEN 1 ELSE 0 END) AS ifend,tb_yunjiainfo.price20gpvalue,tb_yunjiainfo.price40gpvalue,tb_yunjiainfo.price40hqvalue,tb_yunjiainfo.price45hqvalue,tb_user.star_allin "
                sql = sql & "FROM "
                sql = sql & "tb_yunjiainfo,tb_user "
                sql = sql & "WHERE "
                sql = sql & "tb_yunjiainfo.ifEnx=1 AND tb_user.userid = tb_yunjiainfo.useridx "
        ...
        Dim base_url As String = "/en/fcl/list-" & sp & "-" & dp & "-" & cr & "-" & l & "-" & order & "-0-----"
