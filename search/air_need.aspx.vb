...
Partial Class search_air_need
    ...
    Public Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        sql += "WHERE (STYPE = 10) AND (STARTPORT_1 = '99999999') "
        ...
        Else
            starporthot += "<a href=""/search/air_need.aspx"">全部</a> "
        End If
        For i As Integer = 0 To ds.Tables("bbs").Rows.Count - 1
            If sp = ds.Tables("bbs").Rows(i)(0).ToString Then
                starporthot += "<strong>" + ds.Tables("bbs").Rows(i)(0).ToString.ToUpper + "</strong> "
            Else
                starporthot += "<a href=""/search/air_need.aspx?sp=" & Server.UrlEncode(ds.Tables("bbs").Rows(i)(0).ToString.ToUpper) & "&dp=" & dp & "&l=" & l & "&cr=" & cr & """>" + ds.Tables("bbs").Rows(i)(0).ToString.ToUpper + "</a> "
            End If
        ...
        sql += "FROM TB_LOST_PRICE "
        sql += "WHERE (STYPE = 10) AND (DESTPORT_1 = '99999999') "
        ...
        Else
            destporthot += "<a href=""/search/air_need.aspx"">全部</a> "
        End If
        For i As Integer = 0 To ds.Tables("bbs").Rows.Count - 1
            If dp = ds.Tables("bbs").Rows(i)(0).ToString Then
                destporthot += "<strong>" + ds.Tables("bbs").Rows(i)(0).ToString.ToUpper + "</strong> "
            Else
                destporthot += "<a href=""/search/air_need.aspx?sp=" & sp & "&dp=" & Server.UrlEncode(ds.Tables("bbs").Rows(i)(0).ToString.ToUpper) & "&l=" & l & "&cr=" & cr & """>" + ds.Tables("bbs").Rows(i)(0).ToString.ToUpper + "</a> "
            End If
    ...
    Sub LoadTotalCount()
        ...
        Dim base_url As String = "/search/air_need.aspx?sp=" & Server.UrlEncode(sp) & "&dp=" & Server.UrlEncode(dp) & "&l=" & Server.UrlEncode(l) & "&cr=" & Server.UrlEncode(cr) & "&p="
