...
Partial Class search_fcl_need
    ...
    Public Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        'sql += "WHERE (STYPE = 8) AND (STARTPORT_1 = '99999999') "
        ...
        Else
            starporthot += "<a href=""/search/fcl_need.aspx"">全部</a> "
        End If
        ...
        '    Else
        '        starporthot += "<a href=""/search/fcl_need.aspx?sp=" & Server.UrlEncode(ds.Tables("bbs").Rows(i)(0).ToString) & "&dp=" & dp & "&l=" & l & "&cr=" & cr & """>" + ds.Tables("bbs").Rows(i)(0).ToString + "</a> "
        '    End If
        'Next
        For i As Integer = 0 To 7
            ...
            Else
                starporthot += "<a href=""/search/fcl_need.aspx?sp=" & StartportHotArray(i).Split(",")(0) & "&dp=" & dp & "&l=" & l & "&cr=" & cr & """>" + StartportHotArray(i).Split(",")(0) + "</a> "
            End If
        Next
        ...
        sql += "FROM TB_LOST_PRICE "
        sql += "WHERE (STYPE = 8) AND (DESTPORT_1 = '99999999') "
        ...
        Else
            destporthot += "<a href=""/search/fcl_need.aspx"">全部</a> "
        End If
        For i As Integer = 0 To ds.Tables("bbs").Rows.Count - 1
            If dp = ds.Tables("bbs").Rows(i)(0).ToString Then
                destporthot += "<strong>" + ds.Tables("bbs").Rows(i)(0).ToString + "</strong> "
            Else
                destporthot += "<a href=""/search/fcl_need.aspx?sp=" & sp & "&dp=" & Server.UrlEncode(ds.Tables("bbs").Rows(i)(0).ToString) & "&l=" & l & "&cr=" & cr & """>" + ds.Tables("bbs").Rows(i)(0).ToString + "</a> "
            End If
        ...
        sql += "FROM TB_LOST_PRICE WHERE (STYPE = 8) "
        sql += "GROUP BY LINE "
        sql += "ORDER BY c DESC"
        ...
        Else
            linehot += "<a href=""/search/fcl_need.aspx"">全部</a> "
        End If
        For i As Integer = 0 To ds.Tables("bbs").Rows.Count - 1
            If l = ds.Tables("bbs").Rows(i)(0).ToString Then
                linehot += "<strong>" + ds.Tables("bbs").Rows(i)(0).ToString + "</strong> "
            Else
                linehot += "<a href=""/search/fcl_need.aspx?sp=" & sp & "&dp=" & dp & "&l=" & Server.UrlEncode(ds.Tables("bbs").Rows(i)(0).ToString) & "&cr=" & cr & """>" + ds.Tables("bbs").Rows(i)(0).ToString + "</a> "
            End If
    ...
    Sub LoadRecord(ByVal page As String)
        ...
        sql = sql & "WHERE "
        sql = sql & "1=1 AND STYPE = 8 AND STARTPORT_1 <> '99999999' AND DESTPORT_1 <> '99999999' "
        If sp <> "" Then
    ...
    Sub LoadTotalCount()
        ...
        Dim base_url As String = "/search/fcl_need.aspx?sp=" & Server.UrlEncode(sp) & "&dp=" & Server.UrlEncode(dp) & "&l=" & Server.UrlEncode(l) & "&cr=" & Server.UrlEncode(cr) & "&p="
