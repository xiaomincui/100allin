...
Partial Class search_fcl_1
    ...
    Sub LoadRecord(ByVal page As String)
        ...
        If dp = "" And cr = "" Then
            ...
            sql = sql & "(SELECT TOP " & item.ToString & " tb_yunjia.id AS id,tb_yunjia.postdate,tb_user.star_allin,order_adv,(CASE WHEN TB_YunJia.enddate > GETDATE() THEN 1 ELSE 0 END) AS ifend "
            sql = sql & "FROM "
            sql = sql & "tb_yunjia,tb_user "
            sql = sql & "WHERE "
            sql = sql & "1=1 AND tb_user.userid = tb_yunjia.userid "
            ...
            Else
                sql = sql & "ORDER BY ifend desc,tb_yunjia.order_adv DESC,tb_yunjia.postdate DESC "
                sql = sql & ") AS T "
                sql = sql & ") "
                sql = sql & "ORDER BY ifend desc,order_advx DESC,postdate DESC "
            End If
        ...
        Else
            If dp = "" Then
                ...
                Else
                    sql = sql & "ORDER BY ifend desc,V_CARRIER_PRICE.order_advx DESC,tb_user.star_allin desc," + strsort1
                    sql = sql & ") AS T "
                    sql = sql & ") "
                    sql = sql & "ORDER BY ifend desc,order_advx DESC,star_allin desc," + strsort2
                End If
            Else
                sql = "select top 15 "
                sql = sql & "shipcompany,sid,id,startport,title,hangxianmiaoshu,destport,price20gp,price40gp,price40hq,price45hq,price20gpvalue,price40gpvalue,price40hqvalue,price45hqvalue,assurance,"
                ...
                sql = sql & "WHERE id NOT IN "
                sql = sql & "(SELECT id FROM"
                sql = sql & "(SELECT TOP " & item.ToString & " tb_yunjiainfo.id AS id,tb_yunjiainfo.postdatex as postdate,(CASE WHEN tb_yunjiainfo.enddatex > GETDATE() THEN 1 ELSE 0 END) AS ifend,tb_yunjiainfo.price20gpvalue,tb_yunjiainfo.price40gpvalue,tb_yunjiainfo.price40hqvalue,tb_yunjiainfo.price45hqvalue,tb_user.star_allin,tb_yunjiainfo.order_advx "
                sql = sql & "FROM "
                sql = sql & "tb_yunjiainfo,tb_user "
                sql = sql & "WHERE "
                sql = sql & "1=1 AND tb_user.userid = tb_yunjiainfo.useridx "
                ...
                sql = sql & "ORDER BY ifend desc,tb_yunjiainfo.order_advx desc,tb_user.star_allin desc," + strsort1
                sql = sql & ") AS T "
                sql = sql & ") "
                sql = sql & "ORDER BY ifend desc,order_advx desc,star_allin desc," + strsort2
            End If
