...
Partial Class search_fcl_detail
    ...
    Sub LoadDetail()
        ...
        ltrXgLink1.Text = "<a href=""/fcl/list-" & city0 & "-" & LabDestport.Text & "--0-0-0-----1.html"" style=""margin-left:445px;"" target=""_blank"">更多相关运价>></a>"

        ltrXgLink2.Text = "<a href=""/fcl/list-" & city0 & "---0-0-0-----1.html"" style=""margin-left:445px;"" target=""_blank"">更多相关运价>></a>"
        ...
        For i As Integer = 0 To 9
            If LineHotArray(i).IndexOf(Literal6.Text) <> -1 Then
                ltrXgLink2.Text = "<a href=""/fcl/list-" & city0 & "---" & LineHotArray(i).Split(",")(1) & "-0-0-----1.html"" style=""margin-left:445px;"" target=""_blank"">更多相关运价>></a>"
            End If
        Next
        ...
        sqlother = sqlother & "FROM " & detail_table2 & " "
        ...
        'sqlsearch = sqlsearch & "price20gp as price20gp, "
        'sqlsearch = sqlsearch & "price40gp as price40gp, "
        'sqlsearch = sqlsearch & "price40hq as price40hq, "
        'sqlsearch = sqlsearch & "price45hq as price45hq, "
        ...
        'sqlsearch = sqlsearch & "where tb_yunjiainfo.useridx = tb_user.userid and yunjialeixingx = 8 "
        'sqlsearch = sqlsearch & sqlstarport1
        'sqlsearch = sqlsearch & "and destport like '" & ck.CheckRecord(OtherListDestPort) & "' "
        ...
        'sqlsearch = sqlsearch & "tb_yunjia.price45hq1 as price45hq, "
        ...
        If ds.Tables("user").Rows(0)("userid").ToString = "77805" Then
            Literal8.Text = "<a href=""http://www.100allin.com/shop/bloom/"" target=""_blank"" title=""点击查看TA的商铺"" style=""color:293B5E"">" & ds.Tables("user").Rows(0)("CompanyName").ToString & "</a>"
        Else
            If ds.Tables("user").Rows(0)("userid").ToString = "117726" Then
                Literal8.Text = "<a href=""http://www.100allin.com/shop/jituo/"" target=""_blank"" title=""点击查看TA的商铺"" style=""color:293B5E"">" & ds.Tables("user").Rows(0)("CompanyName").ToString & "</a>"
            Else
        ...
        Literal7.Text = ds.Tables("user").Rows(0)("introduction").ToString
        Literal15.Text = ds.Tables("user").Rows(0)("Login2_Adventage_Line").ToString
        Literal16.Text = ds.Tables("user").Rows(0)("Login2_Adventage_Line2").ToString
        Literal17.Text = ds.Tables("user").Rows(0)("Login2_Adventage_Shipper").ToString
        ...
        Else
            If Literal10.Text = "" Then
                Literal10.Text = "拼箱海运"
            Else
                Literal10.Text = Literal10.Text + " | 拼箱海运"
            End If
        End If
        ...
        Else
            If Literal10.Text = "" Then
                Literal10.Text = "空运"
            Else
                Literal10.Text = Literal10.Text + " | 空运"
            End If
        End If
        ...
        Else
            ...
            Literal3.Text = "<meta name=""description"" content=""" & seocr & Literal5.Text & "到" & LabDestport.Text & ",航线描述-" & Literal6.Text & ",由" & ds.Tables("user").Rows(0)("CompanyName").ToString & "发布该运费价格.环球运费网提供最新最全面的国际海运整箱运费运价查询与发布"" />"
        End If
        ...
        If Labshipcompany.ToString.ToLower = "hanjin" Or Labshipcompany.ToString.ToLower = "韩进" Or Labshipcompany.ToString.ToLower = "APL" Or Labshipcompany.ToString.ToLower = "美国总统" Then
            sql = "SELECT userid,price20gp,price40gp,price40hq,realname,phone,id,yunjiaid,shipcompany FROM TB_YunjiaInfo,tb_user WHERE tb_yunjiainfo.useridx = tb_user.userid and (startportx like '%" & vipcity0 & "%' or startportx like '%" & vipcity1 & "%') and destport like '%" & OtherListDestPort & "%' AND yunjialeixingx = 8 AND order_advx <> 0 and enddatex >= { fn NOW() } order by useridx,price20gpvalue "

        Else
            sql = "SELECT userid,price20gp,price40gp,price40hq,realname,phone,id,yunjiaid,shipcompany FROM TB_YunjiaInfo,tb_user WHERE tb_yunjiainfo.useridx = tb_user.userid and (startportx like '%" & vipcity0 & "%' or startportx like '%" & vipcity1 & "%') and destport like '%" & OtherListDestPort & "%' AND yunjialeixingx = 8 AND order_advx <> 0 and enddatex >= { fn NOW() } AND userid <> 52125 order by useridx,price20gpvalue "
        End If
        ...
        Repeater2.DataSource = ds1.Tables("yunjia").DefaultView
        Repeater2.DataBind()
        ...
        If detail_table2 = "TB_YunJiaInfo_bak" Then
            Literal21.Text = "您查看的该条运价的历史快照，<a href=""/fcl/list-" + Literal5.Text + "-" + LabDestport.Text + "--0-0-0-----1.html"" target=""_blank"">您可以查看更多最新 " + Literal5.Text + " 至 " + LabDestport.Text + " 的运价 >></a>"
        End If
    ...
    Public Function GetFreightCompanyLine(ByVal s1 As String, ByVal s2 As String, ByVal l As String) As Data.DataSet
        ...
        If s2 = "" Then
            sql = sql + "WHERE (TB_YunJia.StartPort LIKE '%" & s1 & "%') AND (TB_YunJia.yunjialeixing = 8) AND (TB_YunJia.hangxianmiaoshu LIKE '%" & l & "%') AND (TB_YunJia.enddate >= { fn NOW() }) "
        Else
            sql = sql + "WHERE ((TB_YunJia.StartPort LIKE '%" & s1 & "%') OR (TB_YunJia.StartPort LIKE '%" & s2 & "%')) AND (TB_YunJia.yunjialeixing = 8) AND (TB_YunJia.hangxianmiaoshu LIKE '%" & l & "%') AND (TB_YunJia.enddate >= { fn NOW() }) "
        End If
    ...
    Public Function GetFreightCompanyPort(ByVal s1 As String, ByVal s2 As String, ByVal d As String) As Data.DataSet
        ...
        If s2 = "" Then
            sql = sql + "WHERE (TB_YunJiaInfo.StartPortx LIKE '%" & s1 & "%') AND (TB_YunJiaInfo.DestPort LIKE '%" & d & "%') AND (TB_YunJiaInfo.yunjialeixingx = 8) AND (TB_YunJiaInfo.enddatex >= { fn NOW() }) "
        Else
            sql = sql + "WHERE ((TB_YunJiaInfo.StartPortx LIKE '%" & s1 & "%') OR (TB_YunJiaInfo.StartPortx LIKE '%" & s2 & "%')) AND (TB_YunJiaInfo.DestPort LIKE '%" & d & "%') AND (TB_YunJiaInfo.yunjialeixingx = 8) AND (TB_YunJiaInfo.enddatex >= { fn NOW() }) "
        End If
