...
Partial Class lcllist
    ...
    Sub delete()
        ...
        If id <> "" Then
            ...
            Dim sql As String
            sql = ""
            sql = sql & "delete from tb_yunjia where ID in (" & id & ");delete from TB_YunJiaInfo where YunJiaID in (" & id & ")"
            cn.mdb_exe(sql)
            ...
            tp.CheckTopNew("9", Me.Request.Cookies("user_id").Value.ToString)
        Else
            '没有选择任何要删除的ID，弹出此对话框
    ...
    Sub update()
        ...
        If id <> "" Then
            ...
            cn.mdb_exe(sql)
            sql = ""
            sql = sql & "update TB_YunjiaInfo_Effective_9 set PostDatex=getdate(),enddatex='" + enddate + "',ifend=1 where YunJiaID in (" & id & ")"
            cn.mdb_exe(sql)
            sql = "UPDATE TB_PRICECOUNT SET TB_PRICECOUNT.lclcount = (SELECT COUNT(id) AS yunjiacount FROM tb_yunjiainfo WHERE yunjialeixingx = 9),TB_PRICECOUNT.countdate = getdate() where TB_PRICECOUNT.cid = 1"
            cn.mdb_exe(sql)
