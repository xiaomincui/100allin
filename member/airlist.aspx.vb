...
Partial Class member_airlist
    ...
    Sub LoadRecord(ByVal page As String)
        ...
        If destport = "" Then
            '无目的港搜索的select语句
            sql = "select top 10 "
            sql = sql & "id,startport,title,line,"
            sql = sql & "postdate,"
            sql = sql & "enddate "
            sql = sql & "FROM (SELECT "
            sql = sql & "id,startport,title,line,"
        ...
        Else
            ...
            sql = sql & "enddate "
            sql = sql & "FROM (SELECT "
            sql = sql & "DISTINCT t_airinfo.id as id,t_airinfo.startport as startport,title,line,"
            sql = sql & "postdate,"
            sql = sql & "CONVERT(varchar(10), enddate, 120) as enddate "
            sql = sql & "FROM "
    ...
    Sub update()
        ...
        If id <> "" Then
            ...
            cn.mdb_exe(sql)
            sql = sql & "update TB_YunjiaInfo_Effective_10 set PostDatex=getdate(),enddatex='" + enddate + "' where AIRID in (" & id & ")"
            cn.mdb_exe(sql)
            sql = "UPDATE TB_PRICECOUNT SET TB_PRICECOUNT.aircount = (SELECT COUNT(t_airinfo.id) AS yunjiacount FROM t_airinfo,t_airprice WHERE t_airinfo.id = t_airprice.airid),TB_PRICECOUNT.countdate = getdate() where TB_PRICECOUNT.cid = 1"
            cn.mdb_exe(sql)
