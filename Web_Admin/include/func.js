<!--
function Cong_winOpen(strUrl) { //新开窗口
	var winname = window.open('Cong_newWin', "_blank", 'left=0,top=0,width=50,height=50,toolbar=no, menubar=yes, scrollbars=no, location=no, statusbar=yes, copyhistory=no ,directories=no');
	winname.document.open('text/html', 'replace');
	winname.document.writeln('<meta http-equiv="refresh" content="0;URL='+ strUrl +'">');
	winname.document.close();
}
function Cong_winOpenScroll(strUrl) { //新开窗口
	var winname = window.open('Cong_newWin', "_blank", 'left=0,top=0,width=50,height=50,toolbar=no, menubar=yes, scrollbars=yes, location=no, status=yes, copyhistory=no ,directories=no');
	winname.document.open('text/html', 'replace');
	winname.document.writeln('<meta http-equiv="refresh" content="0;URL='+ strUrl +'">');
	winname.document.close();
}
function Cong_winOpenFree(strUrl) { //新开窗口
	var winname = window.open('Cong_newWin', "_blank", 'left=0,top=0,toolbar=no, menubar=yes, resizable=yes, scrollbars=yes, location=no, status=yes, copyhistory=no ,directories=no');
	winname.document.open('text/html', 'replace');
	winname.document.writeln('<meta http-equiv="refresh" content="0;URL='+ strUrl +'">');
	winname.document.close();
}
function Cong_winOpenSub(strUrl,strSub) { //新开窗口
...
function Cong_ceSub(formName){//ctrl + enter 提交表单
	document.forms(formName).submit();
	Cong_disAllButton();
}

function Cong_getOptDate(formName,objName){//选择日期
	dRes = showModalDialog('../include/calendar.htm', null, 'dialogWidth: 200px; dialogHeight: 220px; center: yes; resizable: no; scroll: yes; status: no;')
...
function Cong_getOptDateLinkage(formName,objName,linkageObjName){//自动更新到期日期

	dRes = showModalDialog('../../Web_Admin/include/calendar.htm', null, 'dialogWidth: 200px; dialogHeight: 220px; center: yes; resizable: no; scroll: yes; status: no;')
	if(dRes){
		document.forms(formName).elements(objName).value=dRes;
		res0=res.split("-")[0];
		res1=res.split("-")[1];
...
function Cong_getSelectID(formName,selectName,objName){//得到select下的value值
...
function Cong_disAllButton(){//Cong_disAllButton所有提交后按钮disablend
...
function Cong_delConfirm(tAdd,fAdd,strConfirm){//确认confirm事件
	var delQus;
...
//确认confirmSub事件
var delCheck=false;
function Cong_delConfirm(strConfirm){
	var delQus;
	delQus=confirm('确认要删除 {( '+strConfirm+' )} 么?');
	if (delQus==true){
		return true;
...
function Cong_validForm(formName,elementsName,strAlert){
	var eNum=new Array();
