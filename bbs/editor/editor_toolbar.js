...
var gTBCmds = ["bold", "italic", "underline", "fontname", "fontsize", "forecolor", (gIsFF)?"hilitecolor":"backcolor", "alignmode", "serial", "indent", "createlink","picture","flash"];

//下面的是字母下标数组，下标为gTBCmds数组
...
function fGenToolBarHtml() {
	...
	var dat= [
		...
		,["tb_"+gTBCmds[10],"530","77","0",partDisp[3],"增加链接"]
		,["tb_"+gTBCmds[11],"610","77","0",partDisp[4],"插入图片"]
		,["tb_"+gTBCmds[12],"690","77","0",partDisp[4],"插入视频"]
	];
	...
	dat = [
		...
		,[
			...
			,[
				 [0, 1, 2]
				,["mutb_"+gTBCmds[12]+"_alert","padding:5px 0 0 0;", "<div unselectable='on' style='margin:0 0 2px 5px;text-align:left;'>视频地址：</div>"]
				,["mutb_"+gTBCmds[12]+"_pic","padding:3px 0 0 0;", "<table unselectable='on' width='100%' border='0' cellspacing='0' cellpadding='0'><tr><td><input type='text' name='flash' id='flash' style='width:320px;' value='http://'></td></tr></table><div style='line-height:25px;'><a href='http://www.100allin.com/bbs/thread-2581-1.html' target='_blank'>教您如何插入视频</a></div><div unselectable='on' style='margin-top:5px;'><input id='mutb_"+gTBCmds[12]+"_ok' class='wd1 btn' type=button value=' 确定 '><input class='wd1 btn' type=button value=' 取消 '></div>"]
			 ]
			,"<div id='$id$' style='display:none;font:12px normal Verdana;width:100%;text-align:center;$s$' unselectable='on'>$c$</div>"
		 ]
	];
	...
	for (var i = gTBCmds.length - 1; i >= 0; i--) {
		if (fIsInObj(o, "mutb_"+gTBCmds[i])) {
			...
			if (tmp) {
				...
				if (o.type == "button" && o.id == "mutb_"+gTBCmds[12]+"_ok") {
					
				  var UpPic = window.document.getElementById('flash').value;
				   
				   if(UpPic != "" && UpPic != null){
					    UpPic = UpPic.replace(/\\/g,"/"); 
						try{
							fTBPInsertflash(URLFieldEncode(UpPic));
						}catch(e){alert(e.description);}
				   }
				}//end button
				//隐藏弹出框
...
function fTBPInsertPic(para) {
	...
	doc.execCommand('InsertImage',false,para);
	var Inputs=doc.all.tags("IMG");
	
	for(var i=0;i<Inputs.length;i++)
	{

		if(Inputs.item(i).src==para.replace(/%2d/g,"-"))
		{
			Inputs[i].onclick="window.open(this.src);";
			Inputs[i].style.cursor="hand";
		}
	}
...
function fTBPInsertflash(para) {
	...
	if(para.indexOf("<")==0)
	{
		var a111=para.indexOf("src")+5;
		var b111=para.indexOf("\"",para.indexOf("src")+5);

		para = para.substring(a111,b111);

		para = "<embed src='"+para+"' quality='high' width='480' height='400' align='middle' allowScriptAccess='sameDomain' type='application/x-shockwave-flash'></embed>";
	}
	//<embed src="http://player.youku.com/player.php/sid/XOTU0NTQ0NDA=/v.swf" quality="high" width="480" height="400" align="middle" allowScriptAccess="sameDomain" type="application/x-shockwave-flash"></embed>
	else
	{
		para = "<embed src='"+para+"' quality='high' width='480' height='400' align='middle' allowScriptAccess='sameDomain' type='application/x-shockwave-flash'></embed>";
	}
