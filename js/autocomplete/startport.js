...
var loadimg = "/js/autocomplete/loading.gif";/*loading图片路径*/
var url = "/js/autocomplete/PostBack/Startport.aspx";
var currentIndex = -1;
/**
 * 动态创建div，并弹出
 * @param objid 对象id
...
function autowords(objid,event)
{
	...
	if(keyCode == 9)closediv(objid);
	if(obj.value.length<1){closediv(objid);return;}/*判断文本框里是否有文字*/
...
{   
    var fc = function(){closediv(objid)}
    var prot_str ="";
    var tar = document.getElementById(objid).value;
    if(tar.length<1){return;}else{
    document.getElementById("autodiv"+objid).innerHTML = "<div id=\"prot_warp\"><img src=\""+loadimg+"\" alt=\"loading\"/>正在智能匹配港口名称，请稍候...</div>";
    ...
    {     
            ...
            {       
                ...
                for (var i = 0; i < txt.length-1; i++) 
                {
                    cn = txt[i].CN;
                    ch = txt[i].CNCH;
                    prot_str +="<a href=\"javascript:void(0);\" onclick=\"document.getElementById('"+objid+"').value='"+ch+"';hidediv('"+objid+"');\" id=\""+ch+"\"><span>"+ch+"</span>"+cn+"</a>";
                }
                var html_str ="";
                ...
                if (typeof document.attachEvent!='undefined'){
                    document.attachEvent("onclick",fc)
                }else{
                    document.addEventListener("click",fc,false);
                }
             },
         onFailure: function(){
