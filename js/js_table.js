/*
	...
	增，删
修改内容：
2010-07-20： 可监测鼠标粘贴数据动作
2010-07-27： 响应方向键↑↓移动焦点
2010-08-04： 屏蔽ctrl+z （IE仅有）
2010-09-27：	输入中文时可以及时响应

2010-09-06
  *參數說明
  	ConDivID 容器id	此參數必選
  	TittleArray	表頭內容	此參數必選
  	Rows	初始行數		除去表頭的行數。默認為 1 行
*/
...
d2: "\u786e\u8ba4\u662f\u5426\u5220\u9664\u6b64\u884c\uff1f\n*\u6b64\u884c\u6709\u6570\u636e",
//提交數據時，textbox id沒指定，提示錯誤（參數配置錯誤！）
d3: "\u53c3\u6578\u914d\u7f6e\u932f\u8aa4\uff01",
//删除行，如果行内有内容，提示是否删除（确认是否删除此行？\n*此行有数据）
d4: "\u786e\u8ba4\u662f\u5426\u5220\u9664\u9009\u4E2D\u884c\uff1f\n*\u9009\u4E2D\u884c\u6709\u6570\u636e"
}
...
function Dtable() {
    ...
    var self = this;
    self.H_top = 0; //表頭距頁頂端距離
    self.RowSum = 1; //行總數		除去表頭，默認總數為1行
    self.RowList = []; //保存被選中行 行號,方便執行多行刪除操作
    self.ColumnSum = 0; //列總數	除去正文內容的列，默認有 checkbox列+操作列
    self.RowRule = []; //保持td寬度 和 td內容形式。 如 checkbox or radio	默認textarea。	規則： 寬度\n形式
    self.txtArr = []; //临时保存表格数据
    self.key = 0; //按键keycode
    self.ToCode = true;//是否加密傳輸
    
    /** 全局 CSS 變量
    over_color 鼠標劃過 行背景變色
    out_color  鼠標移開 行背景色還原
    txt_b_color	文本框編輯狀態下的邊框顏色
    */
    self.css = {
    ...
    this.DrawTable = function() {
        self.ColumnSum = self.TittleArray.length; //列總數
        var Tempstr = "", eid = "";
        var TableHTML = [], Tlist = [];
        var i = 0, j = 0, h = 0;
        TableHTML.push("<table id=\"" + self.ConDivID + "_Table\" class=\"dtable\">");
        //表頭
        TableHTML.push("<thead><tr><th colspan=\"" + self.ColumnSum + "\">");
        TableHTML.push("<div class=\"mdiv\">");
        TableHTML.push("<div id=\"" + self.ConDivID + "_TittleDiv\" class=\"absdiv\">");
        TableHTML.push("<table class=\"TitleTable\">");
        TableHTML.push("<tr style=\"height:25px\">");
        for (i = 0; i < self.ColumnSum; i++) {
            ...
            if (typeof (Tlist.t) != "undefined") {
                switch (Tlist.t) {
                    ...
                    case "span":
						TableHTML.push("<th style=\"width:" + Tlist.w + "px;\">");
                        TableHTML.push("<span id=\"" + eid + "\" " + tclass(Tlist.c) + ">刪除</span>");
                        break;
                    default:
						TableHTML.push("<th style=\"width:" + (Tlist.w+4) + "px;\">");
        ...
        {	
            ...
            for (i = 0; i < self.ColumnSum; i++)//列 循環
            {
                ...
                if (typeof (Tlist.t) != "undefined") {
                    switch (Tlist.t) {
                        ...
                        case "checkbox":
                            ...
                            break;
                        case "span":
                            TableHTML.push("<span id=\"" + eid + "\" " + tclass(Tlist.c) + " >del</span>");
                            break;
                        case "textarea":
                        case "input":
                        default:
    ...
    */
    ...
    this.read_excel = function(obj) {
        var _r = [], _c = [], _rl = 0, _cl = 0, _rs = 0, _cs = 0, i = 0, j = 0, n = 0, m = 0, q = 0;
        ...
        if (_s.split("\t").length > 1 || _s.split("\n").length > 1) {
            ...
            q = parseInt(_cl, 10) + parseInt(_cs, 10);
                ...
                if (typeof (t.t) != "undefined") {
                    switch (t.t) {
                        ...
                        case "span":
                            ...
                            sp.id = d;
                            sp.className = t.c || "";
                            txt = document.createTextNode("del");
                            sp.appendChild(txt);
                            td.appendChild(sp);
                            break;
                        default:
    ...
    this.clearall = function(){
    	var i=0,y=0;
    	var l = self.tbody.getElementsByTagName("textarea");
    	y=l.length;
    	for(i=0;i<y;i++){
    		l[i].value="";
    	}
    ...
    this._CreatDiv = function() {
        ...
        _d.setAttribute("id", "autodiv");
       	_d.style.position = "absolute";
	    _d.style.zIndex = "30";
	    _d.style.overflow = "auto";
	    _d.className = "autodiv";
